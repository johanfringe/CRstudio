...
import { CONTEXT, statusMap, defaultMeta } from './statusMap';
...
export function resolveUserStatus({ user, session, artist, token, originPage }) {
  ...
  if (user && session && !artist) {
    ...
    const isNewByDate = user.created_at && Date.now() - new Date(user.created_at).getTime() < 10 * 60 * 1000;
  ...
  if (token?.isTempUser && !token?.isExpired) {
    ...
    if (!createdAtMs || isNaN(createdAtMs)) {
      console.warn('[CRstudio] NEW_USER_PENDING: ontbrekende of ongeldige createdAt:', createdAtString);
